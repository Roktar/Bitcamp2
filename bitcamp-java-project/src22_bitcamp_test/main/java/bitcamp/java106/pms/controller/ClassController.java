...
public class ClassController implements Controller {
    ...
    public ClassController(Scanner scanner) {
    ...
    void onList() {
        ...
        Iterator<Classroom> list = classroomDao.list();
        while( list.hasNext() ) {
            Classroom classroom = list.next();
    ...
    void onUpdate(String option) {
        ...
        int idx = classroomDao.indexOf(classroom.getNo());
        
        if (Console.confirm("변경하시겠습니까?")) {
            classroomDao.update(idx, updateClassroom);
